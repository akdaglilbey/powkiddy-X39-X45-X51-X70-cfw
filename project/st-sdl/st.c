...
SDL_Surface* screen;
#if defined(MIYOOMINI) || defined(POWKIDDY)
SDL_Surface* screen2;
#endif
...
void sdlinit(void) {
...
#ifdef POWKIDDY
    /* Reset FB pan offset — OWL FB peut rester sur un offset
     * laissé par l'appli précédente (SimpleMenu double buffer etc.) */
	...
	fprintf(stderr, "init joystick");
	SDL_JoystickEventState(SDL_ENABLE);
	int num_joysticks = SDL_NumJoysticks();
	fprintf(stderr, "Found %d joystick(s)\n", num_joysticks);
	SDL_Joystick* joysticks[5]; // ou malloc si plus de 2
	for(int i=0; i<num_joysticks; i++) {
		joysticks[i] = SDL_JoystickOpen(i);
		if(!joysticks[i]) {
			fprintf(stderr, "Failed to open joystick %d\n", i);
		} else {
			fprintf(stderr, "Opened joystick %d: %s\n", i, SDL_JoystickName(i));
		}
	}
#else
	...
	fprintf(stderr, "SDL xw.w: %d xw.h %d xw.fx %d xw.fy %d\r\n", xw.w, xw.h, xw.fx, xw.fy);
    //xw.w = initial_width;
    //xw.h = initial_height;

#if defined(MIYOOMINI)
	fprintf(stderr, "MIYOO\r\n");
	xw.w = initial_width;
	xw.h = initial_height;
	...
	fprintf(stderr, "SetVideoMode ok\r\n");
#if defined(MIYOOMINI) 
	...
	screen2 = SDL_CreateRGBSurface(SDL_SWSURFACE, xw.w, xw.h, 16, screen->format->Rmask, screen->format->Gmask, screen->format->Bmask, screen->format->Amask);
	fprintf(stderr, "CreateRGB ok\n");
#else
	xw.win = SDL_CreateRGBSurface(SDL_SWSURFACE, xw.w, xw.h, 16, screen->format->Rmask, screen->format->Gmask, screen->format->Bmask, screen->format->Amask);
	fprintf(stderr, "CreateRGB ok\n");
#endif
	...
	fprintf(stderr, "thread ok\n");
	expose(NULL);
	fprintf(stderr, "expoese ok\n");
	//vi = SDL_GetVideoInfo();
	...
	fprintf(stderr, "event ok\n");
	SDL_PushEvent(&event);
	fprintf(stderr, "pushevent ok\n");
}
...
kmap(SDLKey k, SDLMod state) {
	...
	for(i = 0; i < LEN(key); i++) {
		...
		if(key[i].k == k && ((state & mask) == mask
				|| (mask == 0 && !state))) {
			fprintf(stderr,"key found index %d %s\r\n", i, key[i].s);
			return (char*)key[i].s;
		}
	}
	fprintf(stderr,"key not found index %d\r\n", i);
	return NULL;
}
...
run(void) {
    ...
    while(SDL_WaitEvent(&ev)) {
		...
		if((ev.type == SDL_KEYDOWN || ev.type == SDL_KEYUP) && ev.key.keysym.mod != (1 << 13)) continue;
        if(ev.type == SDL_KEYDOWN || ev.type == SDL_KEYUP || ev.type == SDL_JOYBUTTONDOWN || ev.type == SDL_JOYBUTTONUP) {
            //take only first joystick as remapping is done in SDL
			if(ev.jbutton.which != 0) continue;
