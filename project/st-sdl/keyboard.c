...
#define EVDEV_BTN_A      0  
#define EVDEV_BTN_B      1 
#define EVDEV_BTN_X      2
#define EVDEV_BTN_Y      3
#define EVDEV_BTN_L1     4
#define EVDEV_BTN_R1     5
#define EVDEV_BTN_L2     6
#define EVDEV_BTN_R2     7
#define EVDEV_BTN_SELECT 8
#define EVDEV_BTN_START  9
#define EVDEV_BTN_MENU   10
#define EVDEV_BTN_VOLUP  11
#define EVDEV_BTN_VOLDOWN 12 
#define EVDEV_BTN_ON     13
/*#else
...
void draw_keyboard(SDL_Surface* surface) {
    ...
    // Calcul du total_length avec scale
    int total_length = -1;
    for(int i = 0; i < NUM_KEYS && syms[0][0][i]; i++) {
        total_length += (1 + strlen(syms[0][0][i])) * 6 * scale; // * scale ici
    }

    int center_x = (surface->w - total_length) / 2;
    int x = center_x;
    int y = surface->h - 8 * scale * NUM_ROWS - 16;  // hauteur * scale
    if(location == 1) y = 16;

    SDL_Rect rect = {x - 4 * scale, y - 3 * scale, total_length + 3 * scale, NUM_ROWS * 8 * scale + 3 * scale};
    ...
    for(int j = 0; j < NUM_ROWS; j++) {
        ...
        for(int i = 0; i < row_length[j]; i++) {
            ...
            SDL_Rect r2 = {x - 2 * scale, y - 1 * scale, length * 6 * scale + 4 * scale, 7 * scale};
            if(toggled[j][i]) {
            ...
            draw_string(surface, syms[shifted][j][i], x, y, text_color); // scale ici
            x += 6 * scale * (length + 1); // largeur * scale
        }
        y += 8 * scale; // hauteur * scale
    }
}
#if 0
void draw_keyboard(SDL_Surface* surface) {
...
int handle_keyboard_event(SDL_Event* event) {
	...
	fprintf(stderr, "handle eventtype: %d\n", event->type);
	if(event->type == SDL_JOYBUTTONDOWN || event->type == SDL_JOYBUTTONUP) {
		int btn = event->jbutton.button;
		fprintf(stderr, "joykey: %d\n", btn);
		fprintf(stderr, "joystick number: %d\n", event->jbutton.which);
        // map SDL_JoyButtonEvent vers SDL_KeyboardEvent
		event->key.keysym.scancode = event->key.keysym.sym = event->key.keysym.mod = event->key.keysym.unicode = 0;
		
        // map la touche physique vers un SDLK artificiel
        switch(btn) {
            case EVDEV_BTN_A:       event->key.keysym.sym = KEY_ENTER;     break;
            case EVDEV_BTN_B:       event->key.keysym.sym = KEY_TOGGLE;    break;
            case EVDEV_BTN_X:       event->key.keysym.sym = KEY_ACTIVATE;  break;
            case EVDEV_BTN_Y:       event->key.keysym.sym = KEY_LOCATION;  break;
            case EVDEV_BTN_L1:      event->key.keysym.sym = KEY_SHIFT;     break;
			case EVDEV_BTN_R1:      event->key.keysym.sym = KEY_BACKSPACE;     break;
			case EVDEV_BTN_L2:      event->key.keysym.sym = KEY_TAB;     break;
            case EVDEV_BTN_R2:      event->key.keysym.sym = KEY_TAB; break;
            case EVDEV_BTN_SELECT:  event->key.keysym.sym = KEY_QUIT;      break;
            case EVDEV_BTN_START:   event->key.keysym.sym = KEY_RETURN;    break;
            case EVDEV_BTN_VOLUP:   event->key.keysym.sym = KEY_ARROW_UP;  break;
            case EVDEV_BTN_VOLDOWN: event->key.keysym.sym = KEY_ARROW_DOWN;break;
			case EVDEV_BTN_ON:		event->key.keysym.sym = KEY_QUIT;break;
			case 14: event->key.keysym.sym = KEY_UP;break;
			case 15: event->key.keysym.sym = KEY_LEFT;break;
			case 16: event->key.keysym.sym = KEY_RIGHT;break;
			case 17: event->key.keysym.sym = KEY_DOWN;break;
...
#define EVDEV_BTN_VOLDOWN 12 
#define EVDEV_BTN_ON     13
            // ajoute le reste si nécessaire
            default:                event->key.keysym.sym = 0;            break;
        }
		...
		fprintf(stderr, "handle eventtype: %d %d %d %d %d %d %d %d %d %d\n", event->key.type,event->key.state,event->type, event->jbutton.button, event->jbutton.type, event->jbutton.which, event->jbutton.state, event->key.keysym.sym,  event->key.keysym.scancode, event->key.keysym.mod);
        // transforme l'événement en “keyboard” pour le reste du code
	...
	if(event->key.type == SDL_KEYDOWN && event->key.state == SDL_PRESSED) {
		fprintf(stderr, "button pressed sym: %d\n", event->key.keysym.sym);
		if(show_help) {
			// do nothing
